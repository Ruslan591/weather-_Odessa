...
**Открытый вопрос на будущее**: у `eumetsat_geocolour_motion.py` (RGB, day/night) station_state тоже может считаться по общему `fc.local_area_mask()` (50км) — не проверено, надо будет свериться по аналогии, если появится похожая жалоба на этот блок.
