...
- **Оркестратор**: `scripts/gh_satellite_pipeline.py` — вызывается GitHub Actions (`satellite_pipeline.yml`) по `workflow_dispatch`, триггерит телефон (`termux-job-scheduler`, job 1001, каждые 15 мин, через `scripts/trigger_gh_pipeline.sh`). [ИЗМЕНЕНО 2026-08-20, см. docs/topics/main_pipeline.md] До этой даты было наоборот: телефон триггерил `full_pipeline.yml`, а тот последним шагом диспетчил этот workflow. Теперь порядок цепочки обратный — этот workflow первый, сам явным диспетчем тянет за собой `full_pipeline.yml` (а тот — `ai_pipeline.yml`). Порядок важен: cloud_forecast → frontal_track → ... → geocolour_motion/ir_motion.
...
## Открытые задачи / На горизонте (обновлено 2026-08-20)

- **[РЕАЛИЗОВАНО 2026-08-19/20] Покраска реальной формы фронта вместо PCA-эллипса (near+west) + легенда в таблице.** По итогам разбора "белые пиксели — мы красим или EUMETSAT" (наши, по палитре CLM_ANCHORS) пользователь предложил красить сами пиксели блоба вместо рисования линии по оси эллипса. Реализован вариант Б целиком (не промежуточный вариант А — отклонён пользователем как некорректный: раскраска текущего блоба по цвету трека ИЗ ПРОШЛОГО цикла давала бы то же рассогласование, только по цвету, а не по форме):
  1. `eumetsat_cloud_forecast.py`: `_significant_blobs()` теперь возвращает `(candidates, pixel_map)` — `pixel_map` (int32, TILE_SIZE×TILE_SIZE, значение=target_id+1) строится в двух проходах (local, затем system поверх — гарантированный приоритет system при перекрытии пикселей). `_save_clm_snapshot()` больше не рисует треки — сохраняет только "полуфабрикат" (контур берега+круг обзора, БЕЗ треков/маркера Одессы) в scratch `_scratch_clm_base.png`; `pixel_map` — в `_scratch_clm_pixelmap.npy`. Оба scratch-файла НЕ коммитятся в git (живут в рамках одного прогона job'а).
  2. `eumetsat_west_watch.py`: `target_id` присваивается сразу после `_detect_frontlike_systems()` (раньше, чем раньше — до этого назначался позже, после ir/gc-подтверждения); своя `pixel_map` из уже существовавшего `_pixel_mask` (раньше выбрасывался, теперь используется). West раньше вообще не рисовал треки на снимке (не фикс, а новая функциональность).
  3. `eumetsat_frontal_track.py`: экспортирует `current_target_id` для каждого трека — target_id ПОСЛЕДНЕЙ точки, гарантированно из ТЕКУЩЕГО кадра (цикл уже отфильтрован по `last_seen == cf_ts_str` до этой строки). Единый механизм для near и west — оба тира матчатся в одном цикле с `cf_ts_str` как референсом.
  4. **Новый скрипт** `eumetsat_render_track_overlay.py` — работает в оркестраторе ПОСЛЕ `frontal_track.py` (свежий `current_target_id` уже есть, лаг в 1 цикл устранён полностью, не только для near, но и для west). Красит пиксели `pixel_map == current_target_id+1` в цвет `FRONTAL_TRACK_COLORS[track_id % 7]`, поверх — маркер Одессы, сохраняет финальный PNG. Если scratch-файлов нет (cloud_forecast/west_watch не писали новых данных в этом цикле — гейт) — тихо пропускает тайл, прежний PNG остаётся как есть. Протестирован синтетически (фейковые scratch + frontal_track.json) — near и west оба красятся верно, пропуск west при отсутствии скратча работает.
  5. `gh_satellite_pipeline.py`: новый шаг `check_eumetsat_render_track_overlay()` вызывается сразу после `check_eumetsat_frontal_track()`.
  6. `nearby_precip.js`: палитра `FRONTAL_TRACK_COLORS` продублирована в JS (порядок и состав — зеркало Python, менять оба разом), цветной кружок перед `#track_id` в обеих таблицах (near+west), пояснение — в title-подсказке заголовка колонки (не отдельным блоком, чтобы не загромождать таблицу).
  Всё провалидировано (`py_compile`+`ast.parse` для Python, `node --check` для JS) и запушено. Проверка "в бою" (реальный трек на снимке) пока не случилась — на утро 2026-08-20 треков нет (`tracks: []`), но 5+ прогонов пайплайна после деплоя прошли `success` без ошибок в новом шаге — не сломал существующий поток.

- **[РЕШЕНО 2026-08-19] "Разница 30 минут между West и Central тайлами на nearby.html" — не рассинхрон данных, а разные единицы времени.** `eumetsat_west_watch.json` писал `"timestamp"` = время САМОГО КАДРА (объявленное сервером EUMETSAT, `server_latest_iso`). `eumetsat_cloud_forecast.json`/`eumetsat_geocolour_motion.json`/`eumetsat_ir_motion.json` писали `"timestamp"` = время ГЕНЕРАЦИИ (`datetime.now()` в момент прогона скрипта) — хотя реальное время кадра (`times[-1]`) было тут же в коде, просто не использовалось для этого поля. Из-за этого Central всегда выглядел свежее, чем был на самом деле; сравнение с West было некорректным (яблоки с апельсинами). Это тот же корень, что и заметка "Central ведёт себя как West" из более ранней записи ниже — но там речь шла про синхронизацию 3 снимков МЕЖДУ СОБОЙ (уже сделано 2026-08-18), а не про формат самого поля timestamp (не было замечено до этого разбора).
  Сделано:
  1. `eumetsat_cloud_forecast.py`, `eumetsat_geocolour_motion.py`, `eumetsat_ir_motion.py`: `"timestamp": now.strftime(...)` → `"timestamp": times[-1]` (4 места правки, все 3 файла).
  2. Побочный найденный баг: гейт 15/10-мин в оркестраторе (`gh_satellite_pipeline.py`, `check_eumetsat_cloud_forecast/geocolour_motion/ir_motion`) парсил `timestamp` жёстким форматом БЕЗ миллисекунд (`%Y-%m-%dT%H:%M:%SZ`), а `times[-1]` — С миллисекундами (`...:00.000Z`) → без фикса гейт бы тихо ловил `ValueError` (проглатывается общим `except Exception: pass`) и пайплайн перезапускал бы эти 3 скрипта на КАЖДОМ прогоне вместо раз в 10-15 мин. Добавлена `_parse_ts_flexible()` — понимает оба формата (нужно на переходный период, пока в репо ещё не перезаписанный файл в старом формате).
  Оба пункта провалидированы (`py_compile`+`ast.parse`) и запушены, 4 файла.
