...
_Последнее обновление: 2026-08-21 10:38 UTC (вынос AI-блока в отдельный workflow + перестройка порядка цепочки: телефон→спутниковый→этот→AI; полностью подтверждено в проде, временный cron-мост закрыт, штатный режим)_
...
## Архитектура цепочки (с 2026-08-20, штатный режим подтверждён 2026-08-21)
...
- **`full_pipeline.yml` и `ai_pipeline.yml` отработали безупречно** — 36/36
  ...
  не только в теории.
- **Найдена, устранена и закрыта дыра:** `satellite_pipeline.yml` не
  запускался ни разу с 18:25 (2026-08-20) до обнаружения (03:17,
  2026-08-21) — **8.5 часов без спутникового мониторинга**. Причина: телефон
  всё это время продолжал дёргать `full_pipeline.yml` напрямую (старый
  `trigger_gh_pipeline.sh` на устройстве), а `full_pipeline.yml` (по новому
  коду) больше не диспетчит спутниковый сам, и его собственный `workflow_run`
  тоже убран этим же рефакторингом — в моменте не осталось ни одного
  источника триггера для спутникового модуля, кроме телефона. Устранено
  тремя шагами: (1) немедленный ручной `workflow_dispatch`, (2) временный
  `schedule: cron: "*/15 * * * *"` добавлен в `satellite_pipeline.yml` как
  мост, (3) **обновление на телефоне подтверждено рабочим 2026-08-21 с
  07:56 UTC** (`event=workflow_dispatch`, `actor=Ruslan591`, интервалы
  ~15 мин, вся цепочка до `ai_pipeline.yml` включительно проходит без
  единого сбоя) — cron-мост убран в 10:38 UTC, штатный режим восстановлен
  полностью.
- **Побочная находка: обычный `git pull` на телефоне непрактичен.**
  Локальный checkout сильно отстал (пайплайны коммитят каждые ~15 мин
  месяцами), `git pull` попытался скачать 155 906 объектов и завис на
  мобильном интернете. Проверка API показала: весь репозиторий на GitHub —
  **~20 ГБ** (`size` в `GET /repos/{owner}/{repo}` = 20 877 129 КБ). Для
  разовой правки одного файла это несоразмерно. Рабочий обходной путь,
  которым решили в этот раз: скачать файл напрямую через `raw.githubusercontent.com`
  с авторизацией существующим `GH_DISPATCH_TOKEN` из `.env`, без единого
  git-действия:
  ```bash
  cd /storage/emulated/0/Documents/weather
  TOKEN=$(grep "^GH_DISPATCH_TOKEN=" .env | cut -d '=' -f2-)
  curl -H "Authorization: token $TOKEN" -o scripts/trigger_gh_pipeline.sh \
    https://raw.githubusercontent.com/ruslan591/weather-_Odessa/main/scripts/trigger_gh_pipeline.sh
  chmod +x scripts/trigger_gh_pipeline.sh
  ```
  Сработало. **20 ГБ репозитория расследовано** (см. «На горизонте» — точная
  причина найдена, дальнейшие шаги пока не предприняты).
...
- **`scripts/trigger_gh_pipeline.sh` выполняется ЛОКАЛЬНО на телефоне**
  ...
  обновления локальной копии файла. Единственный файл в проекте с таким
  свойством (все остальные скрипты выполняются в облаке, на checkout
  GitHub Actions). **Обычный `git pull` для этого непрактичен** — репозиторий
  ~20 ГБ (см. ретроспективу выше), pull на телефоне может зависнуть на
  часы. Рабочий способ обновить именно этот файл — прямая скачка через
  `raw.githubusercontent.com` с `GH_DISPATCH_TOKEN` из `.env`, без единого
  git-действия (команда — см. ретроспективу выше). Подтверждено рабочим
  2026-08-21.

## На горизонте

- **20 ГБ репозитория — причина расследована 2026-08-21, решение не принято.**
  Не баг GitHub, не глюк API — реальные данные. Текущее рабочее дерево HEAD
  всего 123 МБ (`git/trees/main?recursive=1`, 1160 blob-ов) — то есть ~20 ГБ
  почти целиком история: старые версии одних и тех же путей, перезаписанные
  тысячи раз за 5 месяцев (репозиторий создан 2026-03-19), и никогда не
  вычищенные из истории. `.gitattributes` отсутствует — Git LFS нигде не
  используется, все бинарники (видео/картинки/npz) и текстовые JSON-снапшоты
  хранятся как полноразмерные blob-ы на каждый коммит, без дельта-сжатия
  (для бинарников оно и не сработало бы). Точное число коммитов по пути —
  через GraphQL (`repository.ref.target.history(path:...).totalCount`,
  надёжнее постраничного подсчёта через REST `commits?path=`). Главные
  виновники (коммитов × текущий размер ≈ вклад в историю, без поправки на
  сжатие — реальные 20 ГБ уже с учётом сжатия для JSON):
  | Файл | Коммитов | Текущий размер | ≈ вклад |
  |---|---|---|---|
  | `data/ensemble_snapshots_synop.json` | 3381 | 3.3 МБ | ~11 ГБ |
  | `data/ensemble_snapshots_pws.json` | 2445 | 4.4 МБ | ~10.5 ГБ |
  | `data/forecast_video_gemini.mp4` | 947 | 13.6 МБ | ~12.5 ГБ |
  | `data/marine_history.json` | 3609 | 1.6 МБ | ~5.5 ГБ |
  | `data/eumetsat_geocolour_buffer.npz` | 1471 | 1.3 МБ | ~1.9 ГБ |
  Плюс десятки спутниковых PNG/MP4 под `data/anim/` с той же природой (сотни
  коммитов каждая). Суть: `ensemble_snapshots_*.json`/`marine_history.json` —
  не по-настоящему растущие журналы, а файлы, которые перезаписываются
  ЦЕЛИКОМ каждый цикл, поэтому git не может дельта-сжать даже текстовый
  JSON между версиями. Возможные направления (НЕ проработаны, не начинать
  без отдельного обсуждения — история проекта, `git filter-repo`/аналоги
  необратимы без бэкапа): (1) Git LFS для видео/картинок/npz — не решает
  JSON-снапшоты; (2) вообще вынести `ensemble_snapshots_*`/`marine_history`
  из git (объекты меняются каждый цикл целиком — идеальный кандидат для
  внешнего хранилища вместо репозитория); (3) переписать историю
  (`git filter-repo --path ... --invert-paths` или BFG) — сильно ужмёт
  репозиторий, но перепишет все SHA, потребует force-push и пересинхронизации
  всех локальных checkout'ов (включая телефон).
