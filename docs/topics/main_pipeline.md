...
- **Побочная находка: обычный `git pull` на телефоне непрактичен.**
  ...
  **~20 ГБ** (`size` в `GET /repos/{owner}/{repo}` = 20 877 129 КБ). Для
  ...
  ```
  Сработало. **20 ГБ репозитория — отдельная системная проблема**, не
  решённая и не расследованная в этой сессии (см. «На горизонте») — рано
  или поздно упрётесь в лимиты GitHub, а любой будущий `git clone`
  (например, на новом устройстве) будет такой же болью, что и этот `pull`.
...
- **20 ГБ репозитория** (обнаружено 2026-08-21, см. ретроспективу выше) — не решено. Рано или поздно упрётесь в лимиты GitHub; любой будущий `git clone`/полноценный `git pull` на новом или давно не синкавшемся устройстве будет такой же болью, как этот. Вероятная причина — накопленная история мелких data-коммитов от пайплайнов, идущих каждые ~15 мин месяцами. Возможные направления (не проработаны): вынести часто меняющиеся `data/*.json`/картинки из git-истории (Git LFS, или вовсе не в git — отдельное хранилище/artifact), либо периодическая обрезка старой истории. Требует отдельного разбора, не трогать не подумав — история проекта, откат может быть болезненным.
