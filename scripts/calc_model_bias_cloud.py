...
Cloud-версия calc_model_bias.py для запуска в GitHub Actions.
Идентична телефонной версии, кроме BASE_DIR (автоопределение вместо
захардкоженного пути Termux). Логику НЕ менять без зеркального патча
в scripts/calc_model_bias.py (телефонная версия остаётся эталоном).

Читает data/modeldata/YYYY_MM.json.
