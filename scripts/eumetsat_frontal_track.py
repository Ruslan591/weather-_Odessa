...
IR_VETO_SIGMA = -1.0
# Порог "активного противоречия" ИК против GC (2026-08-18, живой случай):
# GC=100% на двух west-кандидатах (aspect_ratio 4.29/2.85, ось С-Ю,
# ~470-520км СЗ — визуально явный вытянутый облачный щит на GeoColour/CLM),
# оба отклонены старой политикой из-за roi_contrast_sigma −0.23/−0.27 —
# ЧУТЬ ниже MIN_CLOUD_CONTRAST_SIGMA=0.8, но НЕ отрицательный сигнал "точно
# не облако", а слабый/нейтральный. Ту же картину дали ВСЕ 13
# local_candidates near-tier в тот момент (ir_confirmed=False у каждого).
# Физика: тонкий перистый/высокослоистый щит — классический предвестник
# фронта за много часов до него — оптически плотный в видимом свете
# (яркий белый на GC), но ПОЛУПРОЗРАЧНЫЙ для теплового ИК 10.5мкм, поэтому
# почти не даёт контраста. Обязательное ИК-подтверждение в такой ситуации
# отсекает именно то, ради чего пайплайн строился.

def _west_confirmed(c):
    """Фильтр по IR/GC ROI-подтверждению — политика ПЕРЕСМОТРЕНА 2026-08-18
    (см. IR_VETO_SIGMA выше) по прямому решению пользователя после разбора
    живого случая: "я думаю надо положиться на тебя... делай".

    СТАРАЯ политика (введена 2026-08-17): требовать подтверждения ОБОИХ
    каналов, если оба доступны. На следующий же день живые данные показали,
    что это отсекает тонкую перистую облачность перед фронтом (см. докстринг
    IR_VETO_SIGMA) — ИК для такой облачности НИКОГДА не подтвердит, не
    потому что это шум, а потому что она физически полупрозрачна для этого
    канала. Жёсткое И было category error: путало "ИК не подтверждает" (что
    для тонкой облачности НОРМАЛЬНО) с "ИК опровергает" (что означает
    реальный шум сегментации — GC ошибочно принял тёплую подстилающую
    поверхность, блик на воде, освещённую суше и т.п.).

    НОВАЯ политика — GC первичен, ИК используется только как ВЕТО при
    сильном отрицательном сигнале:
      - GC доступен и подтвердил → подтверждено, ЕСЛИ ИК не показывает
        сильное противоречие (roi_contrast_sigma <= IR_VETO_SIGMA — ROI
        заметно ТЕПЛЕЕ фона, что для облака физически подозрительно).
        Слабый/нейтральный/недоступный ИК подтверждению не мешает.
      - GC доступен и НЕ подтвердил → не подтверждено (это исходный
        мотивирующий случай 2026-08-17: "3 из 4 CLM-кандидатов НИ ИК, НИ
        GC не подтвердили" — там GC тоже не подтверждал, этот путь
        не изменился, шум по-прежнему отсекается).
      - GC недоступен (слой не скачался в этом цикле), доступен только
        ИК → используем ИК как раньше (единственный доступный канал).
      - Ни один канал не доступен → не подтверждено (как раньше — лучше
        пропустить цикл, чем показать вообще непроверенный кандидат)."""
    ir = c.get("ir_confirmation") or {}
    gc = c.get("gc_confirmation") or {}
    ir_available = bool(ir.get("available"))
    gc_available = bool(gc.get("available"))
    ir_sigma = ir.get("roi_contrast_sigma")

    if gc_available:
        if not gc.get("confirmed"):
            return False
        if ir_available and ir_sigma is not None and ir_sigma <= IR_VETO_SIGMA:
            return False
        return True
    if ir_available:
        return bool(ir.get("confirmed"))
    return False
...
def main():
    ...
    for t in tracks:
        if t["last_seen"] != cf_ts_str:
            continue
        pts = t["points"]
        # Показываем трек ТОЛЬКО когда он полностью укомплектован — та же
        # MIN_POINTS_FOR_VELOCITY, что и порог для расчёта скорости/
        # направления/станции ниже (не два разных порога: "2 для показа"
        # + "3 для скорости" — по предложению пользователя 2026-08-17,
        # после того как трек 93 показался публично с 2 точками и пустыми
        # velocity/movement_bearing/ahead_station/behind_station — сбивало
        # с толку, выглядело как будто их "удалили"/"отфильтровали", хотя
        # они просто ещё не были посчитаны). Теперь либо трек показывается
        # ПОЛНОСТЬЮ (все поля посчитаны), либо не показывается вообще.
        # ВАЖНО: для west-tier это может означать, что треки будут
        # появляться редко или почти никогда — жёсткий IR+GC-фильтр (см.
        # _west_confirmed) даёт редкие подтверждённые кадры, а
        # STALE_TRACK_MINUTES=90 может не оставить времени набрать все 3
        # точки. Если west-треки перестанут появляться вообще — это первое,
        # что нужно будет пересмотреть (см. обсуждение с пользователем
        # 2026-08-17 про STALE_TRACK_MINUTES/MIN_POINTS_FOR_VELOCITY/
        # смягчение _west_confirmed, решение отложено до живых данных).
        ...
        entry = {
            ...
            # Тайл-источник последней точки трека ("near"/"west") — нужен
            # фронтенду, чтобы показывать трек в таблице "своего" аккордея
            # (запрос пользователя 2026-08-17: раздельные таблицы для
            # Западного/Центрального тайлов вместо одной общей). Поле уже
            # считалось в points[-1]["tile"] с самого начала (см. matching
            # выше), просто не пробрасывалось наружу в entry.
