...
def _is_daytime(t_iso):
    # ЕДИНСТВЕННАЯ реализация — в field_motion_common.py (fc.is_daytime),
    # вынесена туда 2026-08-04 (см. коммент там). Локальная копия удалена,
    # чтобы не повторить тот же класс бага, что был с pixel_to_km_offset.
    return fc.is_daytime(t_iso)
...
def _classify_cloud(rgba, is_day):
    ...
    # ДНЕВНОЙ порог перекалиброван 2026-08-11 по историческим SYNOP-сверкам
    # станции 33837 (200 стратифицированных сроков за 2025-2026, сверка на
    # r=25км — см. docs/topics/eumetsat.md, разделы "калибровка по
    # историческим SYNOP" и "sweep v1/v2"). Старый порог (s<0.25, v>0.55)
    # систематически недодетектировал: при N=8-9 (почти сплошная
    # облачность) в среднем видел только 0.54 доли кадра как облако вместо
    # ожидаемых ~1.0. Перебор сетки s_max x v_min показал: s_max почти не
    # влияет, v_min — главный рычаг, компромисс между детектом и ложными
    # срабатываниями на ясном небе. Выбран БАЛАНС (v_min=0.40, s_max=0.35,
    # решение пользователя из трёх вариантов): детект при N=8-9 вырос до
    # 0.76, при этом ложное срабатывание на N=0 (ясно) — всего 1.9%
    # (было 0%), на N=1-2 — 5.5%. Более агрессивный v_min=0.35 давал
    # детект 0.86, но ложные уже 5-16% — сочли риском.
    if is_day:
        cloud = (s < 0.35) & (v > 0.40)
    else:
        # НОЧНОЙ порог перекалиброван 2026-08-12 по историческим SYNOP-сверкам
        # станции 33837 (73 ночных срока из уже собранного calibration_
        # geocolour_synop.jsonl, сверка на r=25км — см. docs/topics/
        # eumetsat.md, раздел "решение — калибруем ночь GeoColour"). Старый
        # порог (v>0.10) давал ~40% ложных срабатываний на ясном ночном небе
        # (N=0) — почти не использовал сигнал V (средняя яркость в ROI
        # монотонно растёт с N: 0.25 при N=0 -> 0.48 при N=8-9). Перебор
        # v_min x s_max показал: s_max не помогает (режет детект сильнее,
        # чем ложные — S не даёт независимой информации после V), v_min —
        # рабочий рычаг. Выбран БАЛАНС v_min=0.20 (решение пользователя из
        # трёх вариантов): ложное на N=0 упало до 5.6% (было ~40%), детект
        # при N=8-9 — 0.51 (более строгие 0.25/0.30 давали 0.46/0.42 при
        # почти нулевом ложном, но менее чувствительны к реальной облачности).
        cloud = _in_hue_range(h, 180, 260) & (s > 0.06) & (v > 0.20)
...
# _save_clean_snapshot()/SNAPSHOT_FILE УБРАНЫ отсюда 2026-08-17 — снимок для
# nearby.html теперь пишет eumetsat_cloud_forecast.py (см.
# _save_synced_gc_ir_snapshots() там), синхронизированный с CLM по времени
# (решение пользователя: "Central: привязать все 3 снимка к одному —
# станет медленнее, зато согласованно"). Если бы обе функции продолжали
# писать один и тот же SNAPSHOT_FILE, результат зависел бы от порядка
# скриптов в пайплайне — было решено оставить ОДНОГО владельца файла.
# Функция самого моушен-анализа (буфер, скорость, area_fraction и т.д.)
# этой правки не касается — она как была на гейте 10 мин, так и осталась.
