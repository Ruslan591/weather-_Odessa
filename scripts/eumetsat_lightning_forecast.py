...
def main():
    ...
    for st in fc.load_system_targets_all():
        ...
        if roi_valid.sum() == 0:
            system_analysis_all.append({
                ...
                "available": False,
                "reason": "ROI вне окна li_afa-кадра или нет данных",
            })
            continue
        lightning_fraction = float(roi_presence[roi_valid].mean()) if roi_valid.any() else 0.0
        system_analysis_all.append({
            "target_id": st["target_id"],
            "available": True,
            "roi_lightning_fraction": round(lightning_fraction, 3),
            "has_lightning": lightning_fraction >= 0.02,
        })
    ...
    for lt in fc.load_local_targets_all():
        ...
        if loc_roi_valid.sum() == 0:
            ...
            continue
        loc_lightning_fraction = float(loc_roi_presence[loc_roi_valid].mean()) if loc_roi_valid.any() else 0.0
        local_analysis_all.append({
            "target_id": lt["target_id"],
            "available": True,
            "roi_lightning_fraction": round(loc_lightning_fraction, 3),
            "has_lightning": loc_lightning_fraction >= 0.02,
        })
