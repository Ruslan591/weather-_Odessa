...
OCR_TEMP_RE = re.compile(r'(\d{1,2}[.,]?\d?)[ \t]{0,3}(?:°[ \t]{0,2}[CС]?|град|[CС])\*?', re.IGNORECASE)
# ВАЖНО: раньше тут был \s*, который матчит и перенос строки БЕЗ ограничения
# длины — из-за этого цифра из искажённого watermark'а (юзернейм, обычно
# оканчивается на цифру/букву типа "s"/"5") могла сцепиться с "С" логотипа
# "TikTok" через несколько строк никак не связанного OCR-мусора между ними
# (реальный кейс: "...tes" -> "...5" + 3 пустые строки + "с` ткТок" -> ложные
# 5.0°C на ролике, где температура вообще не печаталась текстом). Теперь
# цифра и единица измерения обязаны быть в пределах одной строки.
