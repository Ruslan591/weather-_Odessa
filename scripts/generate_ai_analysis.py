...
def build_prompt(days, marine=None, data_time=None):
    ...
    lines = [
        ...
        "300 гПа и т.д.), указывай рядом примерную высоту в КИЛОМЕТРАХ (с одним знаком",
        "после запятой) из переданных данных геопотенциала (ГЕОПОТЕНЦИАЛ / Стратосфера Z),",
        "например: \"на уровне 700 гПа (~3,0 км)\". Не указывай высоту в метрах.",
...
GEMINI_STYLE_INSTRUCTION = (
    ...
    "Температуру и скорость ветра указывай диапазоном (например, 18-19 градусов, "
    "2-3 м/с), а не одним точечным значением. "
    "Уровни давления атмосферы записывай только цифрами с единицей: "
    "850 гПа, 500 гПа, 10 гПа — без пояснений в скобках о высоте."
)
