...
Cloud-версия для GitHub Actions. Идентична телефонной, кроме добавленного
Ubuntu-пути к шрифту DejaVuSans (после apt-get install fonts-dejavu-core).
Логику НЕ менять без зеркального патча в scripts/make_blocks.py.
...
def preprocess_tts(text):
    ...
    text = re.sub(r'\(~(\d+(?:[.,]\d+)?)\s*км\)', r'\1 километра', text)
