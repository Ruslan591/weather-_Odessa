...
def _range_sub(text, unit_pattern, unit_word, allow_decimal=False):
    """Диапазон "X-Y{unit}" -> "X-Y {unit_word}". Предлог "от"/"до" перед
    диапазоном (если есть в исходном тексте) не трогаем вообще — он и так уже
    на месте, конструкция вида "от X до Y" не нужна и звучит вычурно. Дефис
    между числами оставляем как есть: TTS сам читает "21-22" естественной
    парой с небольшой паузой (проверено на примере "7-8" в тексте про УФ-индекс,
    где тот же паттерн без единицы измерения читается без проблем). Раньше
    здесь была сборка "от X до Y", которая для уже предваряемых предлогом
    диапазонов (напр. "опустится до 22-25°C") давала дублирование ("до 22 до
    25 градусов")."""
    num_pat = r'-?\d+(?:[.,]\d+)?' if allow_decimal else r'-?\d+'
    pattern = re.compile(
        r'(' + num_pat + r')\s*[-\u2013\u2014]\s*(' + num_pat + r')\s*' + unit_pattern,
        re.IGNORECASE
    )
    def repl(m):
...
def preprocess_tts(text):
    ...
    # Дробные проценты ("24.5%") нужно ловить ДО общей конвертации дробей ниже,
    # иначе цифры уже превращены в слова и regex для "%" их не находит — знак
    # "%" остаётся неозвученным. Также для нецелых чисел в русском всегда
    # используется родительный падеж ЕДИНСТВЕННОГО числа ("5 десятых процента",
