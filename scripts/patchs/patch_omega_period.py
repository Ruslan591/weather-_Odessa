# PATCH: fc-charts-atmo.js — period analysis for omega chart
# Строки 799-872: добавить period analysis + кнопки дней
...
NEW = '''    // ── period analysis ────────────────────────────────────────────────────
    ...
    if(svgEl){
        ...
        function onMove(clientX){
            if(_activePeriod){
                _activePeriod=null;
                const bar=document.getElementById(\'fcOmegaPeriodBar\');
                if(bar) bar.querySelectorAll(\'button\').forEach(btn=>{btn.style.borderColor=\'#333\';btn.style.color=\'#666\';btn.style.background=\'transparent\';});
            }
    ...
    const periodBtnsHtml=`<div id="fcOmegaPeriodBar" style="display:flex;gap:5px;flex-wrap:wrap;align-items:center;padding:5px 8px 4px;border-top:1px solid #1a1a1a;">
        <span style="font-size:9px;color:#444;flex-shrink:0;line-height:22px;">Период:</span>
        ${dayBuckets.map(b=>`<button onclick="window._omgPC(\'${b.label}\')" style="padding:3px 10px;border-radius:12px;border:1px solid #333;background:transparent;color:#666;font-size:10px;cursor:pointer;touch-action:manipulation;">${b.label}</button>`).join(\'\')}
    </div>`;
    wrap.insertAdjacentHTML(\'beforeend\',periodBtnsHtml);

    window._omgPC=function(label){
        const b=dayBuckets.find(x=>x.label===label);
        if(b) renderPeriodStats(label,b.indices);
    };
...
assert START in src, "START marker not found!"
assert END in src,   "END marker not found!"
...
print(f"Done. Replaced {i2-i1} chars -> {len(NEW)} chars")